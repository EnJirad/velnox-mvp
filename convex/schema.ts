...
const schema = defineSchema(
  {
    ...
    // =========================================================================
    // SESSIONS — Phase 1: Brain Foundation
    // =========================================================================
    // A session groups behavioral events into a coherent browsing instance.
    // Created when the user opens the app/site; ended when they leave or timeout.
    sessions: defineTable({
      userId: v.optional(v.id("users")),
      anonymousId: v.optional(v.string()),
      startedAt: v.number(),
      endedAt: v.optional(v.number()),
      device: v.optional(v.string()), // ANDROID, IOS, WEB, etc.
      platform: v.optional(v.string()), // velshop, velseller, velcenter
    })
    ...
    customerEvents: defineTable({
      ...
      sessionId: v.optional(v.string()), // links events to a browsing session
      type: v.string(), // EVENT_TYPE from customer-memory-core.ts
      entityId: v.optional(v.string()), // Neon product / shop / category id
      value: v.optional(v.string()), // search query / category label
      context: v.optional(v.any()), // extra hints (price, quantity, page, source, position, device)
      createdAt: v.number(),
    })
