...
import { formatThaiDate, formatNumber } from "@/lib/reorder";
import {
  ORDER_STATUS_META,
  formatBaht,
  shortOrderId,
...
export default function Income() {
  const report = useQuery(api.orders.sellerIncome);
        ...
        <div className="mt-7 grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
          <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
            ...
            <p className="mt-2 text-2xl font-bold tabular-nums tracking-tight text-slate-900 sm:text-3xl">
              {report ? formatBaht(report.gross) : "—"}
            </p>
            <p className="mt-1 text-xs text-slate-400">
              {report ? `${formatNumber(report.grossCount)} ชิ้น · ออเดอร์ที่เสร็จสิ้น` : "ออเดอร์ที่เสร็จสิ้น"}
            </p>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
            ...
            <p className="mt-2 text-2xl font-bold tabular-nums tracking-tight text-slate-900 sm:text-3xl">
              {report ? formatBaht(report.returns) : "—"}
            </p>
            <p className="mt-1 text-xs text-slate-400">
              {report
                ? `${pct(report.returnRate)} ของยอดสั่ง · ${formatNumber(report.returnCount)} ชิ้น`
                : "ออเดอร์ที่ยกเลิก"}
            </p>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
            ...
            <p className="mt-2 text-2xl font-bold tabular-nums tracking-tight text-slate-900 sm:text-3xl">
              {report ? `-${formatBaht(report.commission)}` : "—"}
            </p>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
            ...
            <p className="mt-2 text-2xl font-bold tabular-nums tracking-tight text-slate-900 sm:text-3xl">
              {report ? formatBaht(report.payout) : "—"}
            </p>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <ul className="mt-2 space-y-1.5 text-xs leading-5 text-slate-500">
                <li>• เก็บค่าธรรมเนียม <span className="font-semibold text-slate-700">3% ต่อชิ้น</span> จากยอดขายที่เสร็จสิ้น</li>
                <li>
        ...
        <div className="mt-8">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-400">
              {report?.transactions.length ?? 0} รายการ
            </span>
          </div>
