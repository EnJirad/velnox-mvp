...
fun StoreScreen(
    ...
    var storeName by remember { mutableStateOf("ร้านค้า") }
    var isLoading by remember { mutableStateOf(true) }
    var error by remember { mutableStateOf<String?>(null) }

    LaunchedEffect(sellerId) {
        isLoading = true
        EventTracker.get().trackStoreView(sellerId)
        when (val result = productRepo.getProducts(sellerId = sellerId)) {
            is RepoResult.Success -> {
                products = result.data
                storeName = result.data.firstOrNull()?.storeName ?: "ร้านค้า"
            }
            ...
            products.isEmpty() -> EmptyState(
                ...
                items(products, key = { it._id }) { product ->
                    ProductCard(
                        product = product,
                        onClick = {
                            EventTracker.get().trackProductClick(product._id, sellerId)
